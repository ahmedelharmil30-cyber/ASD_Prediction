...
hero(
    eyebrow="Step 1 of 1",
    title="AQ-10 Screening",
    subtitle="Answer honestly — there are no right or wrong answers. This takes about two minutes.",
)
...
with st.form("screening_form"):
    section_label("1 · AQ-10 statements")
    ...
    section_label("3 · Model selection")
    ...
    model_choice = st.selectbox(
        "Run", options=model_choices, index=0, help="Choose a specific model or run all models"
    )
...
if result:
    ...
    with banner_col:
        result_banner(
            ...
            headline="ASD traits likely" if consensus == "YES" else "ASD traits unlikely",
            detail=(
                f"{agreement_pct}% of models agree · AQ-10 total score: "
                f"{result['aq10_total_score']}/10"
            ),
        )
        if best.get("accuracy") is not None:
            st.caption(f"Best model ({best['model_name']}) accuracy: {best['accuracy'] * 100:.1f}%")
        recommendation_box(best["recommendation"])
